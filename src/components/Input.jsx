...
const Input = ({ onChange, className, label, ...inputPrpos }) => {
...
const ListInput = ({ list, onChange, defaultValue, value, ...rest }) => {
  ...
  const candidateItems = itemSelected
    ? []
    : list.filter(({ name }) => {
        return toChosung(name).includes(toChosung(rest.value));
      });
  ...
  const sortedItems = candidateItems.sort((a, b) => {
    ...
    let [aHead, bHead] = [a, b].map(({ name }) => name.slice(0, valueLength));
  ...
  const options = (sortedItems.length || itemSelected ? sortedItems : list).map(
    ({ name, key, value }) => (
  ...
  return (
    <Grid column={itemSelected ? "1" : "1:1.5"}>
      <TextInput onChange={value => textInputOnChange(value)} {...rest} />
      <div className={selectElemClassName}>
        <select className={styles.input} onChange={selectOnChange}>
...
const Select = ({ list, onChange, ...rest }) => {
  let defaultValue;
  const options = list.map(({ name, key, value, selected }) => {
  ...
  return (
    <div className={styles.selectWrapper}>
      <select className={styles.input} onChange={selectOnChange} {...rest}>
        {options}
      </select>
    </div>
  );
...
export { TextInput, NumberInput, ListInput, Select };
